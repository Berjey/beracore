...
import type { Referans } from '@/lib/referans-data';

const HeroCore = dynamic(() => import('@/components/HeroCore'), { ssr: false });

/**
 * Ana sayfanın İSTEMCİ gövdesi.
 *
 * Faz 1.4'e kadar bunun tamamı `src/app/page.tsx` idi ve `'use client'` taşıdığı
 * için veritabanından okuma yapamıyordu. Referanslar panele taşınınca sunucuda
 * okunması gerekti; bu yüzden sayfa ikiye ayrıldı: `page.tsx` sunucuda veriyi
 * okur, burası render eder. Aynı kalıp `Stats`/`ServicePage` için de kullanıldı.
 */
export default function HomeClient({ referanslar }: { referanslar: Referans[] }) {
  ...
  return (
    <>
      ...
      <main id="main" className="relative z-[1]">
        ...
        <Process />
        <Stats />
        <Testimonials referanslar={referanslar} />
        <HomeFaq />
        <CtaBand />
      </main>
