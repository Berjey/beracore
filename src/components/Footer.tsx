...
const ISTANBUL_LINKS = [
  { label: 'İstanbul Web Tasarım', href: '/istanbul/web-tasarim' },
  { label: 'İstanbul Yazılım Firması', href: '/istanbul/yazilim' },
  { label: 'İstanbul E-Ticaret', href: '/istanbul/e-ticaret' },
  { label: 'İstanbul Dijital Pazarlama', href: '/istanbul/dijital-pazarlama' },
  { label: 'İstanbul SEO Ajansı', href: '/istanbul/seo' },
  { label: 'İstanbul Mobil Uygulama', href: '/istanbul/mobil-uygulama' },
];
  ...
  return (
    <footer ref={footerRef} className="relative pt-24 pb-8 px-10 max-md:px-5 overflow-hidden"
      ...
      <div className="max-w-6xl mx-auto relative z-[1]">
        ...
        <div className="mb-6">
          <h3 className="font-body text-[0.68rem] font-bold tracking-[0.3em] uppercase mb-3 text-t3">İstanbul Hizmetleri</h3>
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            {ISTANBUL_LINKS.map((item) => (
              <Link key={item.href} href={item.href} className="font-body text-[0.78rem] text-t2/80 hover:text-accent transition-colors duration-300">{item.label}</Link>
            ))}
          </div>
        </div>
